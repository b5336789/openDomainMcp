...
- Backend focused：`tests/test_task_store.py tests/test_task_worker.py tests/test_task_api.py tests/test_workspace_readiness.py` → **46 passed**。
- Frontend focused：`npm run test:e2e -- tests/smoke.spec.ts` → **4 passed**。
- 後端全測：`PYTHONPATH=src .venv/bin/python -m pytest -q` → **491 passed, 3 skipped**。
- 前端 build：`npm run build` 成功，Vite 產出 53 modules。
