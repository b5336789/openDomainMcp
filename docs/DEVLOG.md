...
## 11. 2026-06-28 — Enterprise Redesign Wave 6A

Wave 6A 將 MCP Publish 既有 retrieval-policy 設定提升為可審計的 publish governance evidence。這一波採 evidence-only policy gate，不改 retrieval 行為、不新增 RBAC secret UI，也不強制本地開發啟用 auth；重點是讓 publish readiness 與 publish decision snapshot 明確反映 policy posture。設計與計畫見 `docs/superpowers/specs/2026-06-28-enterprise-wave-6a-policy-gate-design.md`、`docs/superpowers/plans/2026-06-28-enterprise-wave-6a-policy-gate.md`。

### 11.1 範圍

- **Policy quality gate**：Quality Evidence 新增 `Policy` gate，依 `retrieve_approved_only`、`search_mode`、`rerank_enabled`、`auth_enabled` 與 API key view scopes 產生狀態、分數、summary 與 action。
- **Publish-safe policy posture**：approved-only retrieval 關閉時標為 `needs_review`；unsupported search mode 標為 `needs_review`；approved-only 加上 `hybrid` 或 `vector` search mode 時標為 `ready`。Auth disabled 以 detail 呈現，不阻擋 local/internal publish。
- **Publish decision snapshot**：既有 publish decision gate snapshot 自動收錄 `Policy` gate，舊 decision history 仍可讀且不需要 migration。
- **Frontend refresh**：MCP Publish 在 Save policy 後立即重新抓 `/api/quality/evidence`，讓 Policy gate 不必 reload 頁面即可反映新設定；Quality Lab 與 MCP Publish 延用 generic evidence cards 顯示新 gate。
